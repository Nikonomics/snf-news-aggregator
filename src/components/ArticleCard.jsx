...
function ArticleCard({ article, onAnalyze, onViewDetails, isSaved, onToggleSave }) {
  ...
  const getInsightPreview = () => {
    ...
    return insight.length > 100 ? insight.substring(0, 97) + '...' : insight
  }
  ...
  return (
    <>
      ...
      <div className="gnews-card" onClick={() => setShowFullAnalysis(true)}>
        <div className="gnews-card-content">
          {/* Source badge at top */}
          <div className="gnews-source-row">
            <div className="gnews-source-badge">
              <div className="gnews-source-icon">{article.source.charAt(0).toUpperCase()}</div>
              <span className="gnews-source-name">{article.source}</span>
            </div>
            <div className="gnews-meta-right">
              <span className="gnews-time">{format(new Date(article.date || article.published_date), 'MMM d')}</span>
              <button
                className={`gnews-bookmark-btn ${isSaved ? 'saved' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
          ...
          {/* Main content area */}
          <div className="gnews-main-content">
            <div className="gnews-text-content">
              {/* Headline */}
              <h3 className="gnews-headline">{article.title}</h3>

              {/* AI Insight Preview - One line */}
              {insightPreview && (
                <div className="gnews-insight-preview">
                  <Sparkles size={14} />
                  <span>{insightPreview}</span>
                </div>
              )}

              {/* Metadata row */}
              <div className="gnews-metadata">
                <span className="gnews-badge gnews-badge-category">{article.category || 'General'}</span>
                {article.scope && (
                  <span className="gnews-badge gnews-badge-scope">
                    <MapPin size={10} />
                    {article.scope}
                  </span>
                )}
                {article.relevance_score && (
                  <span className="gnews-badge gnews-badge-relevance">
                    <Star size={10} />
                    {article.relevance_score}
                  </span>
                )}
                <span
                  className="gnews-badge gnews-badge-impact"
      ...
      {/* Full Analysis Modal (unchanged from original) */}
      {showFullAnalysis && (
        <div
          style={{
            position: 'fixed',
